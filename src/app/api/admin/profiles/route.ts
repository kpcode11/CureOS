...
export async function POST(req: Request) {
  ...
  try {
    const { userId, role, specialization, licenseNumber, department } =
      await req.json();
    ...
    switch (role) {
      case "DOCTOR": {
        ...
        if (!specialization || !specialization.trim()) {
          return NextResponse.json(
            { error: "specialization is required for DOCTOR" },
            { status: 400 },
          );
        }
        if (!licenseNumber || !licenseNumber.trim()) {
          return NextResponse.json(
            { error: "licenseNumber is required for DOCTOR" },
            { status: 400 },
          );
        }
      ...
      case "NURSE": {
        ...
        if (!licenseNumber || !licenseNumber.trim()) {
          return NextResponse.json(
            { error: "licenseNumber is required for NURSE" },
            { status: 400 },
          );
        }
        if (!department || !department.trim()) {
          return NextResponse.json(
            { error: "department is required for NURSE" },
            { status: 400 },
          );
        }
      ...
      case "PHARMACIST": {
        ...
        if (!licenseNumber || !licenseNumber.trim()) {
          return NextResponse.json(
            { error: "licenseNumber is required for PHARMACIST" },
            { status: 400 },
          );
        }
        ...
        if (existingProfile) {
          return NextResponse.json(
            { error: "Pharmacist profile already exists for this user" },
            { status: 409 },
          );
        }
      ...
      case "LAB_TECH": {
        ...
        if (!department || !department.trim()) {
          return NextResponse.json(
            { error: "department is required for LAB_TECH" },
            { status: 400 },
          );
        }
        ...
        if (existingProfile) {
          return NextResponse.json(
            { error: "Lab Tech profile already exists for this user" },
            { status: 409 },
          );
        }
      ...
      case "BILLING_OFFICER": {
        ...
        if (existingProfile) {
          return NextResponse.json(
            { error: "Billing Officer profile already exists for this user" },
            { status: 409 },
          );
        }
