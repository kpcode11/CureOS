...
export async function POST(req: Request) {
  ...
  if (role) {
    ...
    if (!mapped) {
      return NextResponse.json(
        { error: `invalid role; allowed: ${ROLE_ENUM.join(", ")}` },
        { status: 400 },
      );
    }
    finalRole = mapped;
  ...
  if (finalRoleEntityId && !finalRole) {
    const re = await prisma.roleEntity.findUnique({
    ...
    if (re) {
      ...
      if (mapped) {
        ...
        console.warn(
          `Unmappable RoleEntity.name -> Role enum: ${re.name} (id=${finalRoleEntityId})`,
        );
      }
    }
  }
  ...
  if (!finalRoleEntityId && finalRole) {
    const re = await prisma.roleEntity.findFirst({
      where: { name: { equals: finalRole, mode: "insensitive" } },
      select: { id: true },
    });
  ...
  try {
    ...
    if (/Expected Role|Invalid `role`/.test(msg)) {
      return NextResponse.json(
        { error: "invalid role value", detail: msg },
        { status: 400 },
      );
    }
