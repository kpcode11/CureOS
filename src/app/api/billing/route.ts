...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!body || typeof body.patientId !== 'string') return NextResponse.json({ error: 'patientId required' }, { status: 400 });
    if (typeof body.amount !== 'number' || Number.isNaN(body.amount)) return NextResponse.json({ error: 'invalid amount' }, { status: 400 });
    ...
    const rec = await prisma.billing.create({ data: { patientId: body.patientId, amount: body.amount, description: body.description ?? '', dueDate } });
