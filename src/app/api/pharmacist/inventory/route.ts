...
export async function POST(req: Request) {
  try {
    ...
    const rec = await prisma.inventory.create({ data: { itemName: body.itemName, category: body.category ?? 'general', quantity: Number(body.quantity ?? 0), minStock: Number(body.minStock ?? 0), unit: body.unit ?? 'ea' } });
