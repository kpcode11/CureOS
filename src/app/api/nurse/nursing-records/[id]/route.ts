...
export async function PATCH(
  ...
  try {
    ...
    const before = await prisma.nursingRecord.findUnique({ where: { id: id } });
    ...
    const updated = await prisma.nursingRecord.update({ where: { id: id }, data: allowed });
    await createAudit({ actorId, action: 'nursing.record.update', resource: 'NursingRecord', resourceId: id, before, after: updated });
