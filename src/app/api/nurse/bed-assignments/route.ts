...
export async function GET(req: Request) {
  ...
  const rows = await prisma.bedAssignment.findMany({
    where: { dischargedAt: null },
    include: {
      bed: true,
      patient: true,
      nurse: { select: { id: true, department: true } },
    },
  ...
  try {
    ...
    if (!bedId || !patientId)
      return NextResponse.json(
        { error: "bedId and patientId required" },
        { status: 400 },
      );
    ...
    if (bed.status !== "AVAILABLE")
      return NextResponse.json({ error: "bed not available" }, { status: 409 });
    ...
    const nurse = userId
      ? await prisma.nurse.findUnique({ where: { userId } })
      : null;

    const [assignment] = await prisma.$transaction([
      prisma.bedAssignment.create({
        data: { bedId, patientId, nurseId: nurse?.id ?? undefined },
      }),
