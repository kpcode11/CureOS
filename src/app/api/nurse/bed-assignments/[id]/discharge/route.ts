...
export async function PATCH(
  ...
  try {
    ...
    const [updatedAssignment, updatedBed] = await prisma.$transaction([
      prisma.bedAssignment.update({ where: { id: id }, data: { dischargedAt } }),
      prisma.bed.update({ where: { id: existing.bedId }, data: { status: 'AVAILABLE' } })
    ]);

    await createAudit({ actorId, action: 'bed.discharge', resource: 'BedAssignment', resourceId: id, before: existing, after: updatedAssignment });
