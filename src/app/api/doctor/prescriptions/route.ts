...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (!medications || !Array.isArray(medications) || medications.length === 0) {
      return NextResponse.json({ error: 'medications is required and must be a non-empty array' }, { status: 400 });
    }
    ...
    for (let i = 0; i < medications.length; i++) {
      ...
      if (!med.frequency || typeof med.frequency !== 'string') {
        return NextResponse.json({ error: `medication[${i}].frequency is required and must be a string` }, { status: 400 });
      }
    }

    if (!instructions || typeof instructions !== 'string' || instructions.trim() === '') {
      return NextResponse.json({ error: 'instructions is required and must be a non-empty string' }, { status: 400 });
    }
