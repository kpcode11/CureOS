  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const queueCount = appointmentsData.filter((appointment) => {
        const appointmentDate = new Date(appointment.dateTime);
        return (
          appointmentDate >= new Date() && appointmentDate <= twoHoursFromNow
        );
      }).length;
      ...
      <div className="min-h-screen bg-slate-50/30">
        <div className="max-w-7xl mx-auto p-6 space-y-8">
          <div className="space-y-4">
            <SkeletonShinyGradient className="h-8 w-64 rounded-lg bg-muted" />
            <SkeletonShinyGradient className="h-4 w-96 rounded-lg bg-muted" />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {Array.from({ length: 2 }).map((_, i) => (
              <SkeletonShinyGradient
                key={i}
                className="h-96 rounded-lg bg-muted"
              />
            ))}
          </div>
        </div>
                  ...
                  <Card className="group border-0 shadow-sm bg-white/60 backdrop-blur-sm hover:shadow-md hover:bg-white/80 transition-all duration-200 cursor-pointer">
                    <CardContent className="p-6">
                      <div className="flex items-start gap-4">
                        <div
                          className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl ring-1 ring-inset ${action.ringColorClass} ${action.iconBackground}`}
                        >
                          <IconComponent
                            className={`h-6 w-6 ${action.iconForeground}`}
                          />
                        </div>
