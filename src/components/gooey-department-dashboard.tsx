...
export default function GooeyDepartmentDashboard() {
          ...
          DEPARTMENTS.map(async (dept) => {
            try {
              const response = await fetch(
                `/api/admin/departments/${dept.title.toLowerCase()}`,
              );
              ...
              return {
                ...dept,
                analyticsData:
                  data.analyticsData || generateMockAnalytics(dept.title),
  ...
  const generateMockAnalytics = (department: string): DepartmentAnalytics[] => {
    ...
    const metricName =
      DEPARTMENTS.find(
        (d) => d.title.toLowerCase() === department.toLowerCase(),
      )?.metrics[0] || "Metric 1";
    ...
    return days.map((date, idx) => ({
      ...
      [DEPARTMENTS.find(
        (d) => d.title.toLowerCase() === department.toLowerCase(),
      )?.metrics[1] || "Metric 2"]: 50 + Math.floor(Math.random() * 30),
      [DEPARTMENTS.find(
        (d) => d.title.toLowerCase() === department.toLowerCase(),
      )?.metrics[2] || "Metric 3"]: 75 + Math.floor(Math.random() * 25),
    }));
  };
