...
export default function UsersManagement() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (step === "profile" && newUserId) {
      ...
      } else if (normalizedRole === "LAB_TECH") {
        if (!profileData.department.trim()) {
                      ...
                      <div className="flex gap-2 justify-end">
                        ...
                        <button
                          ...
                          disabled={
                            !!(
                              user.roleEntityId &&
                              roles.find((r) => r.id === user.roleEntityId)
                                ?.name === "ADMINISTRATOR"
                            )
                          }
            ...
            <div className="border-b border-gray-200 p-6 flex items-center justify-between">
              <h3 className="text-xl font-bold text-gray-900">
                {editingUserId
                  ...
                  : step === "user"
                    ? "Create New User"
                    : `Create ${roles.find((r) => r.id === formData.roleEntityId)?.name || ""} Profile`}
              </h3>
              <button
                onClick={() => {
                  setShowCreateModal(false);
                ...
                <>
                  <div className="p-3 bg-blue-50 border border-blue-200 text-blue-700 rounded text-sm">
                    <p className="font-semibold mb-1">
                      Complete{" "}
                      {roles.find((r) => r.id === formData.roleEntityId)?.name}{" "}
                      Profile
                    </p>
                    <p className="text-xs">
