...
describe('billing routes', () => {
  ...
  it('POST creates a billing record when authorized', async () => {
    ...
    vi.spyOn(prisma.billing as any, 'create').mockResolvedValueOnce(fake as any);
  ...
  it('GET /api/billing supports patientId filter', async () => {
    ...
    vi.spyOn(prisma.billing as any, 'findMany').mockResolvedValueOnce([{ id: 'b1', patientId: 'p1' }] as any);
  ...
  it('PATCH pay marks bill PAID and audits', async () => {
    ...
    vi.spyOn(prisma.billing as any, 'findUnique').mockResolvedValueOnce({ id: 'b1', status: 'PENDING' } as any);
    vi.spyOn(prisma.billing as any, 'update').mockResolvedValueOnce({ id: 'b1', status: 'PAID', paidAt: new Date() } as any);
    const auditSpy = vi.spyOn(auditService, 'createAudit' as any).mockResolvedValueOnce({} as any);
  ...
  it('GET overdue returns pending bills past due date', async () => {
    (vi.spyOn(authLib, 'requirePermission') as any).mockResolvedValueOnce({} as any);
    vi.spyOn(prisma.billing as any, 'findMany').mockResolvedValueOnce([{ id: 'o1', status: 'PENDING', dueDate: new Date(Date.now() - 86400000) }] as any);
  ...
  it('GET patient summary returns patient and summary', async () => {
    (vi.spyOn(authLib, 'requirePermission') as any).mockResolvedValueOnce({} as any);
    vi.spyOn(prisma.patient as any, 'findUnique').mockResolvedValueOnce({ id: 'p1', firstName: 'A', lastName: 'B' } as any);
    vi.spyOn(prisma.billing as any, 'findMany').mockResolvedValueOnce([{ id: 'x', amount: 10, status: 'PENDING' }, { id: 'y', amount: 5, status: 'PAID' }] as any);
  ...
  it('GET billing audit-logs requires audit.read', async () => {
    (vi.spyOn(authLib, 'requirePermission') as any).mockResolvedValueOnce({} as any);
    vi.spyOn(prisma.auditLog as any, 'findMany').mockResolvedValueOnce([{ id: 'al1', action: 'billing.create' }] as any);
